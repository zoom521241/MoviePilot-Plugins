...
class SyncHandler:
    ...
    def process_movie_subscribe(
        ...
        try:
            ...
            # 加载该订阅的历史积分花费（优先使用 source-native identity 作为唯一标识）
            media_source = getattr(subscribe, "media_source", None)
            media_id = getattr(subscribe, "media_id", None)
            if media_source and media_id:
                sub_key = f"{media_source}_{media_id}_movie"
            else:
                sub_key = f"{subscribe.name}_movie"
    ...
    def process_tv_subscribe(
        ...
        try:
            ...
            # 加载该订阅的历史积分花费（优先使用 source-native identity + 季数作为唯一标识）
            media_source = getattr(subscribe, "media_source", None)
            media_id = getattr(subscribe, "media_id", None)
            if media_source and media_id:
                sub_key = f"{media_source}_{media_id}_S{subscribe.season or 1}"
            else:
                sub_key = f"{subscribe.name}_S{subscribe.season or 1}"
            if hasattr(self._search_handler, 'reset_sub_spent_points'):
                self._search_handler.reset_sub_spent_points(sub_key)
