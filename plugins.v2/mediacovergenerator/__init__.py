...
from urllib.parse import urlparse, urlsplit, urlunsplit, unquote_plus, quote, unquote
...
from app.plugins.mediacovergenerator.utils.event_worker import CoverEventWorker
...
class MediaCoverGenerator(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        if not self.stop_service():
            logger.warning("旧封面任务仍在退出，暂不重新初始化，避免并行生成")
            return
    ...
    def stop_task(self):
        """Cancel delayed events and the currently running cover generation."""
        with self._lifecycle_lock:
            pending = self._event_worker.cancel_pending() if self._event_worker else 0
            running = self._update_lock.locked()
            self._event.set()
        if not running and not pending:
            return True, "已取消入库监控任务，当前没有正在执行的封面更新任务"
        logger.info("已清空待处理封面事件并发送停止信号")
...
""",
    ...
    def __ui_tone_component(cls, node: Any) -> Any:
        if isinstance(node, list):
            for item in node:
                cls.__ui_tone_component(item)
            return node
        if not isinstance(node, dict):
            return node

        component = node.get("component")
        props = node.setdefault("props", {}) if component else node.get("props", {})
        ...
        if content is not None:
            cls.__ui_tone_component(content)
        return node
...
''',
    ...
    def update_library_cover(self, event: Event):
        """V3 durable-event delivery must not wait for image generation."""
        ...
        if not mediainfo:
            return
        key = tuple(str(getattr(mediainfo, field, "") or "") for field in
                    ("type", "tmdb_id", "imdb_id", "douban_id", "title_year"))
        return self.__queue_cover_event(("transfer",) + key, ("transfer", mediainfo))

    @eventmanager.register(EventType.WebhookMessage)
    def update_library_cover_webhook(self, event: Event):
        """Queue new-library items; delay and server calls run in our worker."""
        event_info = getattr(event, "event_data", None)
        if not event_info:
            return
        get_value = (event_info.get if isinstance(event_info, dict)
                     else lambda key, default=None: getattr(event_info, key, default))
        if get_value("event") != "library.new":
            return
        server, item_id = get_value("server_name"), get_value("item_id")
        if not server or not item_id or not self._servers or server not in self._servers:
            return
        return self.__queue_cover_event(
            ("webhook", server, str(item_id)),
            ("webhook", (server, item_id, get_value("item_name") or str(item_id))),
        )

    def __queue_cover_event(self, key, payload):
        with self._lifecycle_lock:
            if self._stopping or not self._enabled or not self._transfer_monitor:
                return False
            if self._event_worker is None:
                self._event_worker = CoverEventWorker(
                    self.__process_cover_events,
                    lambda error: logger.error(f"封面事件任务异常: {type(error).__name__}"),
                )
            return self._event_worker.submit(key, payload, self._delay)

    def __process_cover_events(self, events, cancelled):
        """Resolve a burst once and generate at most one cover per library."""
        libraries = {}
        targets = {}
        for kind, payload in events:
            ...
            try:
                if kind == "transfer":
                    mediainfo = payload
                    title_year = getattr(mediainfo, "title_year", "未知媒体")
                    ...
                    if not existsinfo or not existsinfo.itemid:
                        ...
                        existsinfo = self.mschain.media_exists(mediainfo=mediainfo)
                    if not existsinfo or not existsinfo.itemid:
                        logger.warning(f"{title_year} 尚未出现在媒体库中，请检查入库延迟")
                        continue
                    server, item_id = existsinfo.server, existsinfo.itemid
                else:
                    server, item_id, title_year = payload
                if cancelled() or self._stopping:
                    return
                ...
                if cancelled() or self._stopping:
                    return
                if not iteminfo:
                    logger.warning(f"获取 {title_year} 详情失败")
                    continue
                resolved = self.__resolve_cover_library(server, iteminfo, title_year, libraries)
                if resolved:
                    targets[(server, str(resolved[2]))] = (server, item_id, title_year, resolved)
            except Exception as error:
                logger.error(f"入库监控查询失败: {type(error).__name__}")
        for server, item_id, title_year, resolved in targets.values():
            if cancelled() or self._stopping:
                return
            self.__update_library_cover_item(server, item_id, title_year, resolved, cancelled)

    def __begin_update(self):
        with self._lifecycle_lock:
            if self._stopping or not self._update_lock.acquire(blocking=False):
                return False
            self._event.clear()
            self._failed_image_urls.clear()
            return True

    def __resolve_cover_library(self, server, iteminfo, title_year, cache):
        """Resolve V3 library IDs first, falling back to legacy media paths."""
        library_id = None
        library = {}
        ...
        if service:
            if server not in cache:
                cache[server] = self.__get_server_libraries(service)
            libraries = cache[server]
        if libraries:
        ...
        return service, library, library_id

    def __update_library_cover_item(self, server, item_id, title_year, resolved, cancelled):
        # Scheduled full updates share the same ownership lock. Wait here instead
        # of silently losing a new-library event while a scheduled run is busy.
        while True:
            with self._lifecycle_lock:
                if cancelled() or self._stopping:
                    return False
                if self.__begin_update():
                    break
            time.sleep(0.1)
        service, library, library_id = resolved
        ...
        try:
            ...
            latest_item = max(
                ...
                key=lambda item: float(item.get("timestamp", 0)), default=None,
            )
            ...
            try:
                ...
                logger.error(f"初始化字体或翻译时出错: {type(error).__name__}")
            if cancelled() or self._stopping or self._event.is_set():
                return False
            updated = self.__update_library(service, library)
            if updated:
                self.update_cover_history(server=server, library_id=library_id, item_id=item_id)
                logger.info(f"媒体库 {server}：{library['Name']} 封面更新成功（入库监控）")
            elif not cancelled() and not self._event.is_set():
                logger.warning(f"媒体库 {server}：{library['Name']} 封面更新失败（入库监控）")
            return updated
        except Exception as error:
            logger.error(f"入库监控更新媒体库 {library['Name']} 失败: {type(error).__name__}")
            return False
        finally:
            self._monitor_sort = ''
    ...
    def __update_all_libraries(self):
        ...
        if not self._servers:
            return
        if not self.__begin_update():
            logger.warning("封面服务已停止或已有更新任务正在执行，跳过本次任务")
            return "封面服务已停止或已有更新任务正在执行"
        ...
        try:
            ...
            try:
                ...
                logger.error(f"初始化字体时出错: {type(error).__name__}")
            for server, service in self._servers.items():
    ...
    @staticmethod
    def __primary_fallback_url(imageurl):
        """Preserve MP URL placeholders but discard the Backdrop-only image tag."""
        if not imageurl.startswith('[HOST]'):
            return None
        parts = urlsplit(imageurl)
        path, changed = re.subn(r"/Images/Backdrop(?:/\d+)?/?$", "/Images/Primary",
                                parts.path, flags=re.IGNORECASE)
        if not changed:
            return None
        query = '&'.join(part for part in parts.query.split('&')
                         if unquote_plus(part.partition('=')[0]).casefold() != 'tag')
    ...
    def __download_image(self, service, imageurl, library_name, count=None, retries=3, delay=1):
        """Use a local poster if Emby cannot fetch its remote Backdrop image."""
        try:
            safe_library_name = self.__sanitize_filename(library_name)
            ...
            fallback = self.__primary_fallback_url(imageurl)
            current_url = imageurl
            server_name = getattr(service, "name", "")
            # Remember failed Backdrops for this generation, not across runs:
            # several selected episodes may reference the same series image.
            if fallback and (server_name, imageurl) in self._failed_image_urls:
                current_url = fallback
            for attempt in range(max(1, retries)):
                while True:
                    if self._stopping or self._event.is_set():
                        return None
                    try:
                        if '[HOST]' in current_url:
                            if not service:
                                return None
                            response = service.instance.get_data(url=current_url)
                        else:
                            response = RequestUtils().get_res(url=current_url)
                        content = (response.content if response is not None
                                   and response.status_code == 200 else None)
                    except Exception as error:
                        # URLs and exception messages may contain media-server keys.
                        logger.warning(f"媒体图片请求失败: {type(error).__name__}")
                        content = None
                    if self._stopping or self._event.is_set():
                        return None
                    if content:
                        ...
                        return filepath
                    if fallback and current_url == imageurl:
                        self._failed_image_urls.add((server_name, imageurl))
                        current_url = fallback
                        logger.info("背景图不可用，尝试同一媒体项的海报图片")
                        continue
                    break
                logger.warning(f"第 {attempt + 1} 次尝试下载媒体图片失败")
                if attempt + 1 < max(1, retries) and self._event.wait(delay):
                    return None
            logger.error(f"媒体图片下载失败（重试 {max(1, retries)} 次）")
            return None
        except Exception as error:
            logger.error(f"下载图片异常: {type(error).__name__}")
            return None
    ...
    def stop_service(self):
        """Bound shutdown and retain a still-running worker to prevent overlap."""
        with self._lifecycle_lock:
            self._stopping = True
            self._event.set()
        ...
        if scheduler:
            try:
                ...
                logger.error(f"停止封面定时服务失败: {type(error).__name__}")
        deadline = time.monotonic() + 5
        stopped = worker.stop(timeout=5) if worker else True
        idle = self._update_lock.acquire(timeout=max(0, deadline - time.monotonic()))
        if idle:
            self._update_lock.release()
        with self._lifecycle_lock:
            if stopped and idle:
                self._event_worker = None
                return True
        logger.warning("封面任务正在等待当前网络或生成操作结束，保持停止状态")
        return False
